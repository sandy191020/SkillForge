...
import { Award, Download, Share2, Loader2, ArrowLeft } from "lucide-react";
import { getCertificate, getUserName } from "@/lib/database";
...
export default function ViewCertificatePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [certificate, name] = await Promise.all([
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Download
      const fileName = `skillforge-certificate-${certificateData?.role.replace(/\s+/g, '-').toLowerCase()}.pdf`;
      pdf.save(fileName);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `I earned a certificate in ${certificateData?.role} with a score of ${certificateData?.score}%!`,
  ...
  const certificateDate = new Date(certificateData.created_at).toLocaleDateString('en-US', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  });
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">{certificateData.role}</p>
        </div>
      </div>
              ...
              <div className="relative z-10 text-center space-y-6">
                ...
                <div className="flex justify-center">
                  <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-8 py-4 rounded-full shadow-lg">
                    ...
                    <p className="text-3xl font-bold">{certificateData.score}%</p>
                  </div>
                </div>
